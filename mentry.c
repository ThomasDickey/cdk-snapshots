...
CDKMENTRY *newCDKMentry (CDKSCREEN *cdkscreen, int xplace, int yplace, char *title, char *label, chtype fieldAttr, chtype filler, EDisplayType dispType, int fWidth, int fRows, int logicalRows, int min, boolean Box, boolean shadow)
{
   ...
   */
   ...
   mentry->label	= (chtype *)NULL;
   mentry->labelLen	= 0;
   mentry->labelWin	= (WINDOW *)NULL;
   mentry->titleLines	= 0;
   ...
   if (title != (char *)NULL)
   {
      ...
      mentry->titleLines = splitString (title, temp, '\n');
   ...
   */
   ...
   mentry->preProcessFunction	= (PROCESSFN)NULL;
   mentry->preProcessData	= (void *)NULL;
   mentry->postProcessFunction	= (PROCESSFN)NULL;
   mentry->postProcessData	= (void *)NULL;
...
char *injectCDKMentry (CDKMENTRY *mentry, chtype input)
{
   ...
   if (mentry->preProcessFunction != (PROCESSFN)NULL)
   {
   ...
   if (ppReturn != 0)
   {
      ...
      if (mentry->postProcessFunction != (PROCESSFN)NULL)
      {
